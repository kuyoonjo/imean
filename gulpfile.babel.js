// Generated on 2016-07-09 using generator-angular-fullstack 3.7.6
'use strict';
...
gulp.task('wiredep:client', () => {
    return gulp.src(paths.client.mainView)
        .pipe(wiredep({
            ...
            overrides: bowerConfig.overrides,
...
gulp.task('copy:fonts', () => {
    return gulp.src(
        [
            `${clientPath}/bower_components/bootstrap/fonts/**/*`,
            `${clientPath}/bower_components/font-awesome/fonts/**/*`
        ], 
        { dot: true })
...
function call(cmd, args, cb) {
    const child = spawn(cmd, args, { cwd: 'ionic' });
...
function ionicBuild(platform) {
    console.log('Build platform', platform);
    fs.symlink('../dist/client/', 'ionic/www', () => {
        let platforms = {};
        if (fs.existsSync('./ionic/platforms/platforms.json'))
            platforms = require('./ionic/platforms/platforms.json');
        if (platforms[platform]) {
            call('ionic', ['build', platform])
        } else {
            call('ionic', ['platform', 'add', platform], () => {
                call('ionic', ['build', platform])
            });
        }
    });
...
gulp.task('ionic:ios', () => ionicBuild('ios'));
gulp.task('ionic:android', () => ionicBuild('android'));